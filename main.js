...
var ExampleSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Other Vault Path").setDesc("Other Vault Path").addText(
      (text) => text.setPlaceholder("Enter Other Vault Path to send").setValue(this.plugin.settings.DataVault).onChange(async (value) => {
        this.plugin.settings.DataVault = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("External Vault Path").setDesc("External Vault Path").addText(
      (text) => text.setPlaceholder("Enter External folder Path to watch").setValue(this.plugin.settings.externalFolderPath).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName(`Register statusbar commands events ribbons vaultstats`).setDesc("example setting").addToggle(
      (cb) => cb.setValue(true).onChange(async (value) => {
        const currentValue = cb.getValue();
        console.log("Toggle value changed:", currentValue);
        new import_obsidian.Notice("toggled");
      })
    );
    new import_obsidian.Setting(containerEl).setName("Register custom bookmarks view").setDesc("example setting").addToggle(
      (cb) => cb.setValue(true).onChange(async (value) => {
        const currentValue = cb.getValue();
        console.log("Toggle value changed:", currentValue);
        new import_obsidian.Notice("toggled");
    ...
    new import_obsidian.Setting(containerEl).setName("Register custom MarkdownCodeBlockProcessor").setDesc("example setting").addToggle(
      (cb) => cb.setValue(true).onChange(async (value) => {
        const currentValue = cb.getValue();
        console.log("Toggle value changed:", currentValue);
        new import_obsidian.Notice("toggled");
    ...
    new import_obsidian.Setting(containerEl).setName("Register custom ObsidianProtocolHandler").setDesc("example setting").addToggle(
      (cb) => cb.setValue(true).onChange(async (value) => {
        const currentValue = cb.getValue();
        console.log("Toggle value changed:", currentValue);
        new import_obsidian.Notice("toggled");
    ...
    new import_obsidian.Setting(containerEl).setName("Choose folder for USB deployable Obsidian").setDesc("example setting").addButton(
      (cb) => cb.setButtonText("DEPLOY").setWarning().setTooltip("!").onClick(() => {
      })
...
// src/views/bookmarks/bookmarks.ts
var import_obsidian2 = require("obsidian");
var fs = __toESM(require("fs"));
var VIEW_TYPE = "bookmarks-view";
async function openBookmarksView() {
  let leafToReveal = null;
...
var VIEW_TYPE_EXAMPLE = "bookmarks-view";
var BookmarksView = class extends import_obsidian2.ItemView {
  constructor(leaf) {
    super(leaf);
    this.bookmarksFile = new GetBookmarksFile();
  }
  getViewType() {
  ...
  async onOpen() {
    ...
    this.app.workspace.onLayoutReady(() => {
      ...
      const activeItem = this.app.workspace.getActiveViewOfType(import_obsidian2.ItemView);
  ...
  createFolderView(folderContents, parent, depth) {
    for (const item of folderContents) {
      if (item.type === "folder") {
        ...
        const folderContents2 = folderElement.createEl("div", {
          cls: "folder-contents"
        });
        folderContents2.style.display = "none";
        folderName.style.marginLeft = `${depth * 10}px`;
        folderName.style.fontWeight = "bold";
        folderName.style.fontSize = "16px";
        folderName.addEventListener("click", () => {
          this.toggleFolderContents(folderContents2);
        });
        if (depth === 0) {
          const folderElement2 = parent.createEl("div", {
            cls: "space"
          });
        } else {
        }
        this.createFolderView(item.children, folderContents2, depth + 1);
      } else {
  ...
  toggleFolderContents(folderContents) {
    const display = folderContents.style.display;
    folderContents.style.display = display === "none" ? "block" : "none";
  }
  ...
  createTree(bookmarkFolderContents, parent, depth = 1) {
    let i = 1;
    let x = 1;
...
// src/postprocessors/postprocessors.ts
var import_obsidian3 = require("obsidian");
async function initProcessor(source, el, ctx) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (cols[0].trim().startsWith("*")) {
      ...
      const totalPrice = data[currentMonth].map((row2) => parseFloat(row2[1].replace("\xA3", "").trim())).reduce((acc, price) => acc + price, 0);
      row.classList.add("calc-row");
      row.createEl("td", {
        text: "calc"
      });
      row.createEl("td", {
        text: `Total for ${currentMonth}: \xA3${totalPrice}`
      });
      continue;
    } else {
...
async function initCommands() {
  ...
  this.addCommand({
    ...
    editorCallback: (editor) => {
      const now = (0, import_moment.default)().format("DD/MM/YYYY HH:mm");
      editor.replaceRange(` \`${now}\``, editor.getCursor());
      editor.replaceRange(
        "> ",
    ...
    hotkeys: [
      {
        modifiers: ["Mod"],
        // Mod represents the Command key on macOS and the Ctrl key on Windows/Linux
        key: "Shift+J"
      }
    ]
  });
...
// src/move folder file/move files-folders.ts
var fs2 = __toESM(require("fs"));
var path = __toESM(require("path"));
var import_obsidian6 = require("obsidian");
async function startWatching(externalFolderPath) {
  if (fs2.existsSync(externalFolderPath)) {
    fs2.watch(externalFolderPath, (eventType, filename) => {
...
async function moveFileToVault(filePath, destination) {
...
${destination}`);
  ...
  if (fs2.existsSync(filePath)) {
    ...
    try {
      await fs2.promises.rename(filePath, destinationPath);
      new import_obsidian6.Notice(`File moved: ${filePath}`);
      filePath = "";
    } catch (err) {
...
async function initEvents() {
  this.registerEvent(
    this.app.workspace.on("file-open", () => {
      new import_obsidian7.Notice("new file opened");
    })
  );
  this.registerEvent(
    ...
    this.fileMenuHandler = this.app.workspace.on(
      "file-menu",
      (menu, fileOrFolder) => {
        if (fileOrFolder instanceof import_obsidian7.TFolder) {
          menu.addItem((item) => {
            item.setTitle("Open in vscode (folder)").setIcon("search").onClick(() => {
              const vaultPath = this.app.vault.adapter.basePath;
              const folderPath = path2.join(vaultPath, fileOrFolder.path);
              console.log(folderPath);
              window.open(`vscode://file/${folderPath}`);
              new import_obsidian7.Notice("folder open in vscode");
            });
          });
        }
      }
  ...
  this.registerEvent(
    ...
    this.fileMenuHandler = this.app.workspace.on(
      ...
      (menu, fileOrFolder) => {
        if (fileOrFolder instanceof import_obsidian7.TFolder) {
          menu.addItem((item) => {
            item.setTitle("Search in folder").setIcon("search").onClick(() => {
              const folderPath = fileOrFolder.path;
              this.app.internalPlugins.getPluginById("global-search").instance.openGlobalSearch(`path:"${folderPath}"`);
            });
          });
        }
      }
  ...
  this.registerEvent(
    this.fileMenuHandler = this.app.workspace.on(
      "file-menu",
      (menu, fileOrFolder) => {
        if (fileOrFolder instanceof import_obsidian7.TFolder || import_obsidian7.TFile) {
          menu.addItem((item) => {
            item.setTitle(`Send to ${path2.basename(this.settings.DataVault)}`).setIcon("document").onClick(() => {
              if (fileOrFolder.path.endsWith(".md")) {
                moveFileToVault(
                  "C:/Users/Mert Arkin/Desktop/Main PC/03VAULTS/O_BASE/" + fileOrFolder.path,
                  this.DataVault
                );
                console.log(
                  `file sent to vault: ${path2.basename(this.settings.DataVault)}`
                );
...
async function initStatusbar() {
  ...
  item.createEl("span", { text: "Hello World \u{1F30D}" });
...
// src/ribbons/ribbons.ts
var import_obsidian9 = require("obsidian");
var fs3 = __toESM(require("fs"));
async function initRibbons() {
  const ribbonIconEl = this.addRibbonIcon("dice", "Sample Plugin", async () => {
    ...
    new import_obsidian9.Notice(`resetting plugin - ${this.manifest.name}`);
    console.clear();
  });
  ...
  this.addRibbonIcon(
    ...
    () => {
      if (fs3.existsSync(this.externalFolderPath)) {
        new import_obsidian9.Notice(`moving from '${this.externalFolderPath}`);
      } else {
        new import_obsidian9.Notice(`${this.externalFolderPath} doesn't exist!`);
      }
    }
  );
}

// src/metadata/vaultstats.ts
var import_obsidian10 = require("obsidian");
async function initVaultStats() {
  ...
  if (this.app.workspace.getActiveFile() == void 0) {
    return;
  } else {
...
var MyPlugin = class extends import_obsidian11.Plugin {
  async onload() {
    ...
    this.app.workspace.onLayoutReady(async () => {
      ...
      this.settings.DataVault = "C:/Users/Mert Arkin/iCloudDrive/iCloud~md~obsidian/DataVault";
      ...
      this.registerView(VIEW_TYPE2, (leaf) => new BookmarksView(leaf));
